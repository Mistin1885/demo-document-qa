...
Coverage
--------
- create_chat: happy path
- get_chat: found / not-found
- list_chats: ordering by updated_at desc, pagination
- update_chat: partial update (only supplied fields change) / not-found
...
async def test_create_chat_returns_chat_read(db_session: AsyncSession) -> None:
    data = _make_create("My Notebook", description="A great notebook")
    result = await chat_service.create_chat(db_session, data)

    assert isinstance(result, ChatRead)
    assert result.id is not None
    assert result.name == "My Notebook"
    assert result.description == "A great notebook"
    assert result.default_chat_profile_id is None
    assert result.default_embedding_profile_id is None
    assert result.default_reranker_profile_id is None
    assert result.created_at is not None
    assert result.updated_at is not None


@pytest.mark.asyncio
async def test_create_chat_no_description(db_session: AsyncSession) -> None:
    result = await chat_service.create_chat(db_session, _make_create("No Desc"))
...
@pytest.mark.asyncio
async def test_list_chats_returns_list(db_session: AsyncSession) -> None:
...
async def test_list_chats_ordering_newest_first(db_session: AsyncSession) -> None:
    ...
    # Both must appear; newer (Beta) should precede older (Alpha)
    # (updated_at desc, so the last-written row comes first)
    assert b.id in ids
    assert a.id in ids
...
async def test_list_chats_pagination(db_session: AsyncSession) -> None:
    ...
    # IDs on first page must not appear on second
...
@pytest.mark.asyncio
async def test_update_chat_description_only(db_session: AsyncSession) -> None:
    created = await chat_service.create_chat(db_session, _make_create("FixedName"))
    updated = await chat_service.update_chat(
        db_session,
        created.id,
        ChatUpdate(description="New desc"),
    )

    assert updated.name == "FixedName"
...
async def test_delete_chat_removes_record(db_session: AsyncSession) -> None:
    ...
    # Subsequent get must raise ChatNotFound
    with pytest.raises(ChatNotFound):
